...
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>Thoroughbred Moving — HT AIOS v19</title>
  <meta name="description" content="Instant quotes and booking for Thoroughbred Moving. Crews of 2, 3, or 4. Serving Dunnellon, Ocala, Gainesville, The Villages, and Central Florida." />
  <meta name="theme-color" content="#0a102a" />
  <meta name="version" content="v19.0" />
  <link rel="canonical" href="/" />
  <meta property="og:title" content="Thoroughbred Moving — Instant Quote" />
  <meta property="og:description" content="Fast quotes. Easy booking. Reliable local movers in Central Florida." />
  ...
  <style>
    :root{
      ...
      --text:#e7ecff; --muted:#9fb0ff99; --accent:#6ee7ff; --accent2:#74f7c5;
      --ok:#36d399; --warn:#fbbf24; --danger:#f87171; --border:#273261; --chip:#11225b;
      --shadow: 0 12px 40px rgba(0,0,0,.35); --radius:16px; --focus:#7dd3fc;
    }
    *{box-sizing:border-box}
    html,body{height:100%}
    body{margin:0;background:
    ...
    /* Layout */
    header{position:sticky;top:0;z-index:50;background:rgba(10,16,42,.7);backdrop-filter: blur(10px); border-bottom:1px solid var(--border)}
    nav{max-width:1200px;margin:0 auto;display:flex;align-items:center;gap:16px;padding:12px 16px}
    .brand{display:flex;align-items:center;gap:12px}
    .logo{width:36px;height:36px;border-radius:10px;background:conic-gradient(from 200deg at 50% 50%,var(--accent),#77a1ff 30%, #9e7bff 60%, var(--accent2));box-shadow: inset 0 0 18px #ffffff40, 0 6px 18px #0008}
    h1{font-size:18px;margin:0;letter-spacing:.2px}
    ...
    .tabs{margin-left:auto;display:flex;gap:8px;flex-wrap:wrap}
    .tab{padding:8px 12px;border:1px solid var(--border);color:var(--text);border-radius:10px;background:#0b1440;cursor:pointer;user-select:none}
    .tab[aria-selected="true"], .tab.active{background:linear-gradient(180deg,#10215f,#0b1440);box-shadow: inset 0 0 0 1px #2a3d85}
...
<body>
  ...
  <script>
    ...
    function saveNote(){ const v = $('#pNote').value.trim(); if(!v) return; localStorage.setItem('lastNote', v); toast('Note saved'); }
